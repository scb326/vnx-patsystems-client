...
public class ClientAPI {
    ...
    public ConcurrentHashMap<String, ExternalFeed> subscriberMap;
    public OrdersHashMap<String, Order> orderMap;
    public String workingDir;

    public boolean isLogOffCalled;
    protected ConfigInfo configInfo;
    public ExternalFeed externalFeed;
    ...
    public ClientAPI(ExternalFeed externalFeed, ConfigInfo configInfo) {
        try {
        	this.configInfo = configInfo;
        	this.externalFeed = externalFeed;
        	
            workingDir = System.getProperty("user.dir");
            workingDir = System.getProperty("user.dir") + "\\lib\\";
            ...
            this.configInfo = configInfo;
            externalFeed.mainForm.AddLog("Initializing the API");
        }
        catch (Throwable e) {
            e.printStackTrace();
    ...
    protected void initialiseClient() throws Exception {
        ...
        subscriberMap = new ConcurrentHashMap<String, ExternalFeed>();
        
        System.load(workingDir + wrapperFileName);
        
        doInitialise();
        doSetDebugFlags(configInfo.DebugFlag);
        
        doSetHostHandShakePeriods(20,30);
        doSetPriceHandShakePeriods(20,30);
        
        String[] hosts = {configInfo.HostServer};
        String[] prices = {configInfo.PriceServer};
        doOpenConnections(hosts, prices);
        Thread.sleep(3000);
        
        doLogin(configInfo.Username, configInfo.Password, "", false, true);
    ...
    public void doInitialise() throws Exception {
    	check(
    		initialise(
    			workingDir, workingDir, false, false, false, configInfo.ApplicationID, 
    			true, "1.0", configInfo.License
    		), 
    		"doInitialise"
    	);
    }
    ...
    public void doLogin(
        ...
        this.logFile.addMessage(Uties.NowGetTime() + " call doLogin: ");
        System.out.println(Uties.NowGetTime() + " call doLogin: ");
        externalFeed.mainForm.AddLog("Call doLogin");
    }
    ...
    public void doLogoff() throws IOException {
    	...
    	externalFeed.mainForm.AddLog("call doLogoff");
